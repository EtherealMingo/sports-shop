...
// services/wire-mock.ts — 线材管理 Mock 数据
...
const mockWires: API.Wire[] = [
  ...
  { id: 7, brand: '王子', model: 'Pro', spec: '1.30mm', color: '绿色', type: 'tennis', minStock: 2, currentStock: 2, avgCost: 28.00, createTime: '2026-06-01T00:00:00' },
  { id: 8, brand: '李宁', model: 'N55', spec: '0.68mm', color: '白色', type: 'badminton', minStock: 5, currentStock: 5, avgCost: 16.00, createTime: '2026-06-01T00:00:00' },
];
...
const mockSuppliers: API.Supplier[] = [
  ...
  { id: 4, name: '二手器材回收商', contact: '赵师傅', phone: '135-0000-4444', paymentMethod: 'cash', remark: '价格浮动较大' },
];
...
const mockPurchases: API.Purchase[] = [
  ...
  { id: 6, wireId: 5, wireName: 'Luxilon ALU Power', supplierId: 3, supplierName: '广州网球用品专卖', purchaseDate: '2026-06-03', quantity: 3, unitPrice: 35.00, totalPrice: 105.00, paymentMethod: 'cash', batchNo: 'LX-20260603-001' },
];
...
export const mockApi = {
  ...
  addWaste: (data: API.WasteForm) => {
    const wire = mockWires.find(w => w.id === data.wireId);
    const reasonMap: Record<string, string> = { break: '断裂', knot: '打结', human: '人为损坏', other: '其他' };
    const newWaste: API.Waste = {
      ...data, id: Date.now(),
      wireName: wire ? `${wire.brand} ${wire.model}` : '',
      reasonLabel: reasonMap[data.reason] || data.reason,
      createTime: new Date().toISOString(),
    };
  ...
  getInventoryList: () => {
    const inventory: API.InventoryItem[] = mockWires.map(w => {
      const totalPurchased = mockPurchases.filter(p => p.wireId === w.id).reduce((sum, p) => sum + (p.quantity || 0), 0);
      const totalUsed = mockUsages.filter(u => u.wireId === w.id).reduce((sum, u) => sum + (u.quantity || 0), 0);
      const totalWasted = mockWastes.filter(wa => wa.wireId === w.id).reduce((sum, wa) => sum + (wa.quantity || 0), 0);
      ...
      return {
        ...
        avgCost: w.avgCost, isLowStock: currentStock <= (w.minStock || 0),
      };
    });
  ...
  getDailyReport: (date?: string) => {
    ...
    const report: API.DailyReport[] = [
      ...dayUsages.map(u => ({ date: targetDate, wireId: u.wireId, wireName: u.wireName, usedQty: u.quantity, cost: (u.quantity || 0) * 15 })),
      ...dayWastes.map(w => ({ date: targetDate, wireId: w.wireId, wireName: w.wireName, wastedQty: w.quantity, cost: (w.quantity || 0) * 15 })),
    ];
  ...
  getSupplierCompare: (wireId?: number) => {
    ...
    list.forEach(p => {
      ...
      if (!grouped[key]) grouped[key] = { wireId: p.wireId, wireName: p.wireName, supplierId: p.supplierId, supplierName: p.supplierName, totalQty: 0, totalCost: 0, lastDate: p.purchaseDate };
      grouped[key].totalQty += p.quantity || 0;
      grouped[key].totalCost += p.totalPrice || 0;
      if (p.purchaseDate && p.purchaseDate > grouped[key].lastDate) grouped[key].lastDate = p.purchaseDate;
    });
    const compare: API.SupplierCompare[] = Object.values(grouped).map(g => ({
      ...
      avgPrice: g.totalQty > 0 ? g.totalCost / g.totalQty : 0, totalQty: g.totalQty, lastPurchaseDate: g.lastDate,
    }));
  ...
  getWastageReport: (startDate: string, endDate: string) => {
    ...
    const report: API.WastageReport[] = wireIds.map(wireId => {
      ...
      const totalWasted = rangeWastes.filter(w => w.wireId === wireId).reduce((s, w) => s + (w.quantity || 0), 0);
      const totalUsed = rangeUsages.filter(u => u.wireId === wireId).reduce((s, u) => s + (u.quantity || 0), 0);
  ...
  getMonthlySummary: (month: string) => {
    const monthPurchases = mockPurchases.filter(p => p.purchaseDate && p.purchaseDate.startsWith(month));
    const monthUsages = mockUsages.filter(u => u.usageDate && u.usageDate.startsWith(month));
    const monthWastes = mockWastes.filter(w => w.wasteDate && w.wasteDate.startsWith(month));
    const totalPurchaseCost = monthPurchases.reduce((s, p) => s + (p.totalPrice || 0), 0);
    const totalUsageQty = monthUsages.reduce((s, u) => s + (u.quantity || 0), 0);
    const totalWasteQty = monthWastes.reduce((s, w) => s + (w.quantity || 0), 0);
