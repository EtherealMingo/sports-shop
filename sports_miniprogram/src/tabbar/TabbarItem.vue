...
<template>
  <view class="flex flex-col items-center justify-center">
    ...
    <template v-if="item.iconType === 'unocss' || item.iconType === 'iconfont'">
      <view :class="[item.icon, isBulge ? 'text-60px text-white' : 'text-18px']" :style="{ color: isBulge ? '#FFFFFF' : undefined }" />
    </template>
    <template v-if="item.iconType === 'image'">
      <image :src="getImageByIndex(index, item)" mode="scaleToFill" :class="isBulge ? 'h-80px w-80px' : 'h-20px w-20px'" />
    </template>
    <view v-if="!isBulge" class="mt-1px text-10px" :style="{ color: tabbarStore.curIdx === index ? '#2A2A26' : '#8A8A84' }">
      {{ item.text }}
    </view>
    <!-- 角标显示 -->
